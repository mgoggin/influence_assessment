...
class Identity::PasswordResets::EditView < ApplicationView
  def initialize(sid:, player: nil)
    @sid = sid
    @player = player
  end
  ...
  def template
    render AuthWrapperComponent.new do |wrapper|
      ...
      wrapper.explanation { "Type a new password in the fields below." }

      render FormErrorComponent.new(record: @player)

      form_with(url: identity_password_reset_path, method: :patch, class: "space-y-4 md:space-y-6") do |form|
        form.hidden_field :sid, value: @sid
